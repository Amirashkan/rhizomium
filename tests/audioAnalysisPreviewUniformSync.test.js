...
// from the main renderer's). The Audio Analysis level/kick/trig outputs are advanced every frame by
// AudioAnalysisProcessor and written only into the MAIN uniform manager, so the preview's copies of
// `<id>.level/.kick/.trig` stayed at their compile-time default (0). A Circle whose radius is
...
// (__kickLevel / __kickValue / __kickTrig) before each render, mirroring _syncHoldUniforms() for Hold.
...
describe('Audio Analysis live values sync into a fragment preview', () => {
  ...
  it('overwrites <id>.level/.kick/.trig with the live node values', () => {
    ...
    expect(um.uniformValues.get('27.trig')).toBeCloseTo(1, 6);
    expect(um.uniformValues.get('9.radius')).toBe(0.5); // unrelated key untouched
  });

  it('does not touch level/kick/trig keys that belong to a non-Audio node', () => {
  ...
  it('hash changes when the referenced level (pin 0) moves, not only the kick', () => {
    const audio = { id: '27', kind: 'AudioAnalysis', params: {},
      __kickLevel: 0.2, __kickValue: 0.3, __kickTrig: 0 };
    ...
    audio.__kickLevel = 0.85; // only the level changed
