...
export class ShaderPreviewManager {
  ...
  isVisualNode(nodeOrKind) {
    ...
    // CPU-driven scalar Input nodes (Trigger, Hold, Count, Wave) take an input but their output is
    // a single uniform VALUE, not a per-pixel field — so the scalar-with-inputs heuristic below
    // would wrongly mark them visual and show a flat numeric swatch by default. They belong on the
    // numeric path (hidden-by-default thumbnail) alongside Time / ConstFloat / Random Value.
    // (Wave's input is a sync pulse, which drives WHEN its cycle restarts, not a per-pixel field.)
