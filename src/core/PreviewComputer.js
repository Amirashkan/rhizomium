    ...
    try {
      ...
      const evaluate = (target) => {
        ...
        try {
          ...
          // _evaluateParam exposes this node's own parameters to the expression scope, so one
          // parameter can bind to a sibling ("Scale Y = =scaleX").
          this._paramScopeNode = target;
...
// Thumbnail rendering methods (continued)
  ...
  _isClockDrivenNode(node) {
    ...
    // Wave is a free-running LFO: it sweeps its cycle from the clock alone, and none of its
    // parameters name `time`, so only the kind check catches it.
    ...
    if (kind === 'time' || kind === 'randomvalue' || kind === 'wave') return true;
