(function () {
  ...
  function getGraph() {
    return window.graph || (window.editor && window.editor.graph) || null;
  }
  ...
  function readPos(n) {
    if (n && n.position && typeof n.position.x === "number")
  ...
  function readPins(n, key) {
    var src =
      (n && n[key]) ||
      (n && (key === "inputPins" ? n.inputs : n.outputs)) ||
      [];
    return (src || [])
      .filter(function (x) {
        return !!x;
  ...
  function exportJSON(extra) {
    ...
    var nodes =
      g && Array.isArray(g.nodes)
        ...
        : [];
    var conns =
      g && Array.isArray(g.connections)
        ? g.connections.map(function (c) {
            function part(obj, side) {
              if (obj && obj.nodeId)
                return {
                  nodeId: obj.nodeId,
                  pinId: obj.pinId || obj.id || obj.name || side,
                };
              if (c[side] && c[side].nodeId)
                return {
                  nodeId: c[side].nodeId,
                  pinId: c[side].pinId || c[side].id || c[side].name || side,
                };
              return { nodeId: "unknown", pinId: side };
            }
            return {
              from: part(c.from || c, "from"),
              to: part(c.to || c, "to"),
            };
          })
        : [];
    var view = {
      pan: {
        x: e && typeof e.panX === "number" ? e.panX : 0,
        y: e && typeof e.panY === "number" ? e.panY : 0,
      },
      zoom: e && typeof e.zoom === "number" ? e.zoom : 1,
    };
    return {
      app: "Rhizomium-Web",
      version: 1,
  ...
  function downloadJSON(data, filename) {
    ...
    a.download = filename || "rhizomium-project.json";
  ...
  function importJSON(json) {
    ...
    (json.nodes || []).forEach(function (n) {
      var kind =
        ...
        n.type ||
        n.nodeType ||
        (n.meta && (n.meta.kind || n.meta.type)) ||
        "Unknown";
      var def = NodeDefs[kind] || null;
      var inCount =
        def && typeof def.inputs === "number"
          ? def.inputs
          : (n.inputPins && n.inputPins.length) ||
            (n.inputs && n.inputs.length) ||
            0;
      var outCount =
        def && Array.isArray(def.pinsOut)
          ? def.pinsOut.length
          : (n.outputPins && n.outputPins.length) || 1;
      var x =
      ...
      if (node.kind === "ConstFloat" && typeof params.Value === "number")
        node.value = params.Value;
      if (node.kind === "ConstVec2") {
        if (typeof params.x === "number") node.xv = params.x;
        if (typeof params.y === "number") node.yv = params.y;
      }
      if (node.kind === "Expr") {
        if (typeof (n.expr || params.expr) === "string")
          node.expr = n.expr || params.expr;
        else if (n.expressions && typeof n.expressions.expr === "string")
          node.expr = n.expressions.expr;
      }
      if (node.kind === "CircleField") {
        node.props = node.props || {};
        if (typeof params.R !== "undefined")
          node.props.radius = Number(params.R);
        if (typeof params.E !== "undefined")
          node.props.epsilon = Number(params.E);
    ...
    function pinIndex(node, pinId, isInput) {
      ...
      if (Array.isArray(arr)) {
        var i = arr.findIndex(function (p) {
          return p && (p.label === pinId || p === pinId);
        });
        if (i >= 0) return i;
      }
      if (pinId === "A") return 0;
      if (pinId === "B") return 1;
      var asNum = parseInt(pinId, 10);
    ...
    (json.connections || []).forEach(function (c) {
      ...
      var fromN = byId[String(fromId)];
      var toN = byId[String(toId)];
      if (!fromN || !toN) return;
      var fp = pinIndex(
        fromN,
        c.from && (c.from.pin || c.from.pinId || c.from.name),
        false,
      );
      var tp = pinIndex(
        toN,
        c.to && (c.to.pin || c.to.pinId || c.to.name),
        true,
      );
      conns.push({
        from: { nodeId: fromN.id, pin: fp },
        to: { nodeId: toN.id, pin: tp },
      });
      try {
        toN.inputs[tp] = { from: fromN.id, pin: fp };
      } catch (e) {}
    });
    ...
    if (json.view) {
      ...
      if (e && "zoom" in e)
        e.zoom = typeof json.view.zoom === "number" ? json.view.zoom : e.zoom;
    ...
    try {
      if (typeof window.__afterImportHook === "function") {
        window.__afterImportHook();
        try {
          if (window.__syncShaderNow) window.__syncShaderNow();
        } catch (e) {}
      }
    } catch (e) {}
    try {
      if (typeof window.computePreviews === "function") {
        window.computePreviews(getGraph());
      }
    } catch (e) {}
    if (e && typeof e.invalidate === "function") e.invalidate();
  ...
  function mountPill() {
    ...
    ["Export", "SaveLocal", "LoadLocal", "Import"].forEach(function (label) {
      ...
      if (label === "Export")
        b.onclick = function () {
          downloadJSON(exportJSON(), "rhizomium-project.json");
        };
      if (label === "SaveLocal")
        b.onclick = function () {
          localStorage.setItem(
            "rhizomium.autosave.v1",
            JSON.stringify(exportJSON({ autosave: true })),
          );
        };
      if (label === "LoadLocal")
        b.onclick = function () {
          var raw = localStorage.getItem("rhizomium.autosave.v1");
          if (!raw) return;
      ...
      if (label === "Import")
        b.onclick = function () {
          ...
          i.onchange = async function () {
            if (i.files[0]) await importFromFile(i.files[0]);
            i.remove();
          };
  ...
  function attachHotkeys() {
    if (__hotkeysAttached) return;
    __hotkeysAttached = true;
    window.addEventListener(
      ...
      function (ev) {
        ...
        if (k === "l") {
          ev.preventDefault();
          var raw = localStorage.getItem("rhizomium.autosave.v1");
          if (raw) importJSON(JSON.parse(raw));
        }

        if (k === "a") {
          ev.preventDefault();
          var ed = getEditor();
          if (ed && ed.selectAll) ed.selectAll();
        }
        if (k === "d") {
          ev.preventDefault();
          var ed = getEditor();
          if (ed && ed.duplicateSelected) ed.duplicateSelected();
        }
      },
      true,
    );
  ...
  function rhizoKeyArrows(ev) {
    ...
    if (ev.key === "ArrowLeft") {
      ev.preventDefault();
      ed.moveSelection && ed.moveSelection(-step, 0);
    }
    ...
    if (ev.key === "ArrowUp") {
      ev.preventDefault();
      ed.moveSelection && ed.moveSelection(0, -step);
    }
    if (ev.key === "ArrowDown") {
      ev.preventDefault();
      ed.moveSelection && ed.moveSelection(0, step);
  ...
  function probePaths(root) {
    var candidates = [];
    function add(o) {
      if (o && candidates.indexOf(o) === -1) candidates.push(o);
    }
    var names = [
      "editor",
      "app",
